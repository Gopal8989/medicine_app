...
class Menu extends React.Component {
  ...
  Menu = (e) => {
    ...
    // alert("bhjdhjdf");
    axios
      .post("http://localhost:5000/api/medicinesname/this.state.fields", this.state.fields)
      .then((res) => {
    ...
    <div>
      <nav class="navbar navbar-expand-lg navbar-light bg-light">
        ...
        <div class="mr collapse navbar-collapse" id="navbarSupportedContent">
          ...
          {this.state.loggedIn == false ? (    <li class="nav-item">
            <Link to="/login">
      ...
      {this.state.data.map((data, i) =>  <div className="table-responsive-xs" key={i}>
      {this.state.show == false ? (   <h5 type="button"  onClick={() => this.medicinename(i)} id={i} value={data.id}>{data.medicine_name}</h5>):(null)}</div>)}
        <div className="table-responsive-xs"><table class="table">
       {this.state.show == true ? ( <thead>
            <tr>
